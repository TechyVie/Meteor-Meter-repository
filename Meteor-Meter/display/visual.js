  ...
  let currentZoom = 1; // Track current zoom scale
  ...
  const zoom = d3.zoom()
    ...
    .on("zoom", function(event) {
      ...
      // Optionally, update any pins to resize on zoom
      g.selectAll(".pin")
        .attr("r", 8 / currentZoom); // Dynamically update pin size
    });
  ...
  svg.on("click", function(event) {
    ...
    g.append("circle")
      ...
      .attr("cx", invX)
      .attr("cy", invY)
      .attr("r", 8 / currentZoom) // Pin radius scales with zoom
      .attr("fill", "#c21807")
      .attr("stroke", "#fff")
      .attr("stroke-width", 2);
